classdef (Abstract) BaseClient < handle & matlab.mixin.CustomDisplay
    % BASECLIENT Base class for RESTful ebrains.dataproxy services.
    ...
    properties (Constant)
        ...
        cookies = ebrains.dataproxy.CookieJar(fileparts(mfilename('fullpath')));
    end

    methods
        function obj = BaseClient(options)
            % ebrains.dataproxy.BaseClient constructor to be called from 
            ...
            if ~isfield(options,"configFile") && isfile(which("ebrains.dataproxy.Client.Settings.json"))
                options.configFile = which("ebrains.dataproxy.Client.Settings.json");
            end
        ...
        function  [request, httpOptions, uri] = requestAuth(obj, authNames, request, httpOptions, uri)
            ...
            switch authName
                case "HTTPBearer"
                    ...
                    request.Header(end+1) = matlab.net.http.field.AuthorizationField("Authorization","Bearer " + obj.bearerToken);
                otherwise
                    error("ebrains:dataproxy:UnknownAuthorization", "Operation requested an authentication method which was not specified in the OpenAPI spec.")
            end
        end
