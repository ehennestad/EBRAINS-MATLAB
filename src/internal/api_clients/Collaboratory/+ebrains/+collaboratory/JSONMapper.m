classdef (Abstract) JSONMapper < handle
    % ebrains.collaboratory.JSONMapper base class - adds JSON serialization and deserialization.
    ...
    % call the ebrains.collaboratory.JSONMapper constructor from their constructor:
    % 
    ...
    % ebrains.collaboratory.JSONMapper.
    %
    % Annotations can be added to properties as "validation functions".
    %
    % ebrains.collaboratory.JSONMapper Methods:
    %
    ...
    properties (Access=private)
        MATLABProperties ebrains.collaboratory.JSONPropertyInfo
    end
    ...
    methods (Static)
        function fn = fieldName(~,fn)
            % FIELDNAME ebrains.collaboratory.JSONMapper Annotation
            ...
            %
            %   properties
            %       some_field {ebrains.collaboratory.JSONMapper.fieldName(some_field,"some.field")}
            %   end
            %
        end
        ...
        function JSONArray(~)
            % JSONARRAY ebrains.collaboratory.JSONMapper Annotation
        ...
        function out = epochDatetime(in,options)
            % EPOCHDATETIME ebrains.collaboratory.JSONMapper Annotation
            ...
            %       start_date {ebrains.collaboratory.JSONMapper.epochDatetime}
            %       % end_date is UNIX timestamp in milliseconds
            %       end_date {ebrains.collaboratory.JSONMapper.epochDatetime(end_date,'TicksPerSecond',1000)}
            %   end
            arguments
                in
                options.TimeZone = '' 
            ...
            if isstring(in) % From JSON to datetime
                ...
                try
                    ...
                    fprintf(2,'Error in ebrains.collaboratory.JSONMapper: %s\n\n',ME.message);
                end                    
        ...
        function out = stringDatetime(in,format,options)
            % STRINGDATETIME ebrains.collaboratory.JSONMapper Annotation
            % When working with datetime fields either epochDatetime or
            % stringDatetime annotation is required to specify how the
            % datetime is encoded in JSON. This must be the first
            % annotation.
            ...
            %
            %   properties
            %       start_date {ebrains.collaboratory.JSONMapper.stringDatetime(start_date,'yyyy-MM-dd''T''HH:mm:ss')}
            %   end
            arguments
                in
                format string
            ...
            elseif isdatetime(in) % From datetime to (JSON) string
                ...
                out = string(in);
            elseif isnumeric(in)
                out = datetime(in/1000, 'ConvertFrom', 'posixtime');
            end
        end
        ...
        function ConstructorArgument(arg)
            ...
            % constructor to be used when working with nested ebrains.collaboratory.JSONMapper
            % derived classes.
    ...
    methods
        function obj = JSONMapper(s,inputs)
            % ebrains.collaboratory.JSONMapper Constructor. Call this from
            ...
            obj.MATLABProperties = ebrains.collaboratory.JSONPropertyInfo.getPropertyInfo(obj);
        ...
        function obj = fromJSON(obj,json)
            try
                ...
                % in the hierarchy derive from ebrains.collaboratory.JSONMapper.
                %
                % Example:
                %   obj = myClass;
                    ...
                    for currProp = obj(1).MATLABProperties
                        ...
                        if curElement.has(currProp.jName) && ~curElement.get(currProp.jName).isJsonNull
                            ...
                            switch currProp.dataType
                                case {?datetime}
                                    val = arrayfun(currProp.dtConversionFunction,getScalarOrArray(curVal,'double'));
                                ...
                                case {?ebrains.collaboratory.JSONMapperMap}
                                    map = ebrains.collaboratory.JSONMapperMap;
                                    it = curVal.entrySet.iterator;
                                    while it.hasNext
                                        kv = it.next;
                                        map(char(kv.getKey)) = char(kv.getValue.getAsString());
        ...
        function json = jsonencode(obj,raw)
                ...
                for currProp = obj(arrayIndex).MATLABProperties
                    ...
                    switch currProp.dataType
                        ...
                        case {?uint64}
                            ...
                            jObject.add(currProp.jName,ebrains.collaboratory.JSONMapper.MATLABGSON.toJsonTree(val));
                        case {?containers.Map}
                            vals = obj(arrayIndex).(currProp.mName).values;
                            keys = obj(arrayIndex).(currProp.mName).keys;
                            m = com.google.gson.JsonObject;
                            for i=1:length(vals)
                                m.add(keys{i},ebrains.collaboratory.JSONMapper.MATLABGSON.toJsonTree(vals{i}));
                            end
                            jObject.add(currProp.jName,m);
                        case {?meta.class, ?ebrains.collaboratory.JSONMapperMap} % free form
            ...
            if isArray
                if raw
                    ...
                    json = char(ebrains.collaboratory.JSONMapper.MATLABGSON.toJson(arr));
                end
            else
                if raw
                    json = jObject;
                else
                    json = char(ebrains.collaboratory.JSONMapper.MATLABGSON.toJson(jObject));
                end
            end
        end
...
function out = getJSONScalarOrArray(val,forceArray)
    ...
    if forceArray && ~isa(val,'com.google.gson.JsonArray') && length(val) == 1
        ...
        out.add(ebrains.collaboratory.JSONMapper.MATLABGSON.toJsonTree(val));
    else
        out = ebrains.collaboratory.JSONMapper.MATLABGSON.toJsonTree(val);
    end
end
...
function val = getScalarOrArray(curVal,type)
    ...
    val = ebrains.collaboratory.JSONMapper.MATLABGSON.fromJson(curVal,t);
    if type == "string"
        val = string(val);
    end
