classdef (Abstract) JSONEnum
    % JSONEnum Base class for enumerations when working with ebrains.collaboratory.JSONMapper
    % When adding enumeration properties to ebrains.collaboratory.JSONMapper objects, the custom
    ...
    methods
        ...
        function v = fromJSON(obj,json)
            ...
            v = vals(strcmp(string([vals.JSONValue]),char(json)));
            if isempty(v)
                error("JSONMapper:JSONEnum:invalid",'"%s" is not a valid enumeration value for enumeration type "%s"',json,class(obj));
            end
        end
    end
end