classdef Cache < ebrains.collaboratory.BaseClient
    ...
    %
    %   Cache - Constructor
    %   clearCache - Clear a specific cache by key
    %   clearCache1 - Clear every caches
    %
    ...
    % Class methods
    methods
        function obj = Cache(options)
            % Cache Constructor, creates a Cache instance.
            ...
            %   client = ebrains.collaboratory.api.Cache();
            %
            %   % Create a client for alternative server/base URI
            %   client = ebrains.collaboratory.api.Cache("serverUri","https://example.com:1234/api/");
            %
            %   % Create a client loading configuration options from 
            %   % JSON configuration file
            %   client = ebrains.collaboratory.api.Cache("configFile","myconfig.json");
            %
            %   % Create a client with alternative HTTPOptions and an API key
            %   client = ebrains.collaboratory.api.Cache("httpOptions",...
        ...
        function [code, result, response] = clearCache(obj, cacheKey)
            % clearCache Clear a specific cache by key
            % Clear a specific cache by key, only accessible for XWikiAdmin
            %
            % Required parameters:
            %   cacheKey - No description provided, Type: string
            %
            ...
            %   200: success
            %   401: Login required.
            %   403: This API can only be used by an XWiki admin or by the collab admin
            %   404: The cache for the provided key doesn''t exist
            %
            % Returns: 
            %
            ...
            arguments
              obj ebrains.collaboratory.api.Cache
              cacheKey string
            end
            ...
            if ismember("application/json",specAcceptHeaders)
                ...
                error("ebrains:collaboratory:api:clearCache:UnsupportedMediaType","Generated OpenAPI Classes only support 'application/json' MediaType.\n" + ...
                    "Operation '%s' does not support this. It may be possible to call this operation by first editing the generated code.","clearCache")
            end
            ...
            uri.EncodedPath = uri.EncodedPath + "/v1/cache/{cacheKey}/clear";

            % Substitute path parameters
            uri.Path(uri.Path == "{" + "cacheKey" +"}") = cacheKey;
            ...
            % Call preSend
            [request, httpOptions, uri] = obj.preSend("clearCache", request, httpOptions, uri);
            ...
            % Call postSend
            response = obj.postSend("clearCache", response, request, uri, httpOptions);
            ...
            switch (code)
                ...
                otherwise % Unexpected output, not declared in spec
                    ...
                    if (int32(response.StatusCode) < 200 || int32(response.StatusCode) >= 300)
                        ...
                        warning("ebrains:collaboratory:api:clearCache:UndocumentedResponse","Operation '%s' returned an undocumented response code '%d'.\n" + ...
                            "Response Body is returned as raw data.","clearCache",code);
                    end
        ...
        function [code, result, response] = clearCache1(obj)
            % clearCache1 Clear every caches
            % Clear every caches, only accessible for XWikiAdmin
            %
            ...
            if ismember("application/json",specAcceptHeaders)
                ...
                error("ebrains:collaboratory:api:clearCache1:UnsupportedMediaType","Generated OpenAPI Classes only support 'application/json' MediaType.\n" + ...
                    "Operation '%s' does not support this. It may be possible to call this operation by first editing the generated code.","clearCache1")
            end
            ...
            % Append the operation end-point
            uri.EncodedPath = uri.EncodedPath + "/v1/cache/clear";
            ...
            % Call preSend
            [request, httpOptions, uri] = obj.preSend("clearCache1", request, httpOptions, uri);
            ...
            % Call postSend
            response = obj.postSend("clearCache1", response, request, uri, httpOptions);
            ...
            switch (code)
                ...
                otherwise % Unexpected output, not declared in spec
                    ...
                    if (int32(response.StatusCode) < 200 || int32(response.StatusCode) >= 300)
                        % Others will throw a warning
                        warning("ebrains:collaboratory:api:clearCache1:UndocumentedResponse","Operation '%s' returned an undocumented response code '%d'.\n" + ...
                            "Response Body is returned as raw data.","clearCache1",code);
                    end
                    % Return the raw body data
                    result = response.Body.Data;
            end
        
        end % clearCache1 method
