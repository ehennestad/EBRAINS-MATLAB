classdef JSONPropertyInfo < handle
    % JSONPROPERTYINFO class used by ebrains.collaboratory.JSONMapper internally
    ...
    methods (Static)
        function props = getPropertyInfo(obj)
            ...
            ps = properties(obj);
            N = length(ps);
            props = ebrains.collaboratory.JSONPropertyInfo.empty(N,0);
            for i = 1:N
                ...
                if isempty(attrs) % If there are no attributes
                    ...
                    props(i).isArray = any(strcmp(attrs,'ebrains.collaboratory.JSONMapper.JSONArray'));
                    % Check for fieldName attribute
                    fi = find(contains(attrs,"ebrains.collaboratory.JSONMapper.fieldName"));
                    if isempty(fi) 
